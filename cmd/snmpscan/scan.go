...
func NewScanCmd() *cobra.Command {
	...
	var scanCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			r, err := c.Scan(context.Background(), &scanV1.ScanRequest{Range: o.Range, SnmpSettings: &snmpSettings, Oids: oids})
			if err != nil {
				cmd.PrintErr(err)
				return
			}
			...
			for {
				...
				default:
					for _, pdu := range stream.GetPdus() {
						t.AppendRow(table.Row{pdu.GetName(), pdu.GetValue().GetStringValue(), pdu.GetKind(), pdu.GetOid()})
					}
					t.AppendSeparator()
					def_format = true
				}
	...
	scanCmd.Flags().StringVarP(&o.Output, "output format", "o", "", "Output result with format, support json")
